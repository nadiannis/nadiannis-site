...
_Thread_ adalah proses tunggal yang dapat digunakan program untuk menjalankan _tasks_.
Secara default, JavaScript itu _single-threaded_, yang berarti hanya dapat menjalankan satu _task_ dalam satu waktu pada _single thread_ (_main thread_).
...
Karena JavaScript itu _single-threaded_, maka JavaScript hanya memiliki satu _Call Stack_.

Seperti struktur data _stack_ pada umumnya, *Call Stack* menerapkan konsep LIFO (_Last in, First out_).
Jika suatu fungsi dipanggil, maka fungsi tersebut akan masuk ke dalam _Call Stack_.
Ketika fungsi sudah me-_return_ suatu nilai atau sudah selesai dieksekusi, maka fungsi tersebut akan keluar dari _Call Stack_.
...
Hal-hal tersebut bukan bagian dari _JavaScript Engine_ sehingga _task_ dapat dijalankan secara _asynchronous_ di dalam Web API (atau C/C++ API).
...
Perhatikan kembali contoh kode konsep _asynchronous_ sebelumnya. Fungsi `b()` dipanggil setelah fungsi `a()`, maka fungsi `b()` masuk ke dalam _Call Stack_ setelah fungsi `a()` keluar.
...
_Callback function_ yang merupakan argumen fungsi `setTimeout()` ditambahkan ke dalam Web API. _Timer_ pun mulai dijalankan saat _callback function_ ditambahkan ke dalam Web API.
Kemudian di dalam _Call Stack_, fungsi `setTimeout()` dan `b()` keluar dari _Call Stack_.

Setelah _callback function_ ditambahkan ke dalam Web API & fungsi `b()` keluar dari _Call Stack_, maka fungsi `c()` dapat masuk ke dalam _Call Stack_, dieksekusi, menjalankan `console.log()` sehingga tampil huruf c, kemudian fungsi `c()` keluar dari _Call Stack_.
...
Jika _Call Stack_ kosong (artinya fungsi-fungsi sebelumnya sudah selesai dieksekusi & keluar dari _Call Stack_), maka _item_ pertama di dalam _Callback Queue_ ditambahkan ke dalam _Call Stack_ menggunakan _Event Loop_.
